...
int char_memitemdata_to_sql(const struct item items[], int max, int id, storage_type tableswitch, uint8 stor_id) {
	std::string buf;
	SqlStmt* stmt;
		...
		for( i = 0; i < max; ++i )
		{
			...
			&&  items[i].unique_id == item.unique_id
				...
				else
				{
					...
					if (tableswitch == TABLE_INVENTORY)
						buf += string_sprintf(", `favorite`='%d'", items[i].favorite);
	...
	for( i = 0; i < max; ++i )
	{
		...
		if (tableswitch == TABLE_INVENTORY)
			buf += string_sprintf(", '%d'", items[i].favorite);
		for( j = 0; j < MAX_SLOTS; ++j )
...
bool char_memitemdata_from_sql(struct s_storage* p, int max, int id, storage_type tableswitch, uint8 stor_id) {
	std::string buf;
	SqlStmt* stmt;
	int i, offset = 0;
	...
	buf += string_sprintf(" FROM `%s` WHERE `%s`=? ORDER BY `nameid`", tablename, selectoption );
